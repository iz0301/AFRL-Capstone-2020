...
val_dataset = ImageFolder(test_dir,  transform=transforms.Compose(
    [transforms.Grayscale(), transforms.ToTensor()]))
val_dataloader = DataLoader(
    val_dataset, batch_size=batch_size, shuffle=True, num_workers=5)
...
pre_filter = nn.Conv2d(in_channels=1, out_channels=1,
                       kernel_size=3, bias=False)
pre_filter.weight.data = pre_filter_weights
# Let the filter weights change during training
...
cnn = ConvolutionalNN([IMSZ, IMSZ, 1], conv_layers,
                      np.asarray([5000, 500, 50]), n_classes, nn.RReLU)
...
for epoch in range(num_epochs):
    # Keeping track of time if we want to show a progress bar or soemthing later
    end_time = time.time()
    totalCorrect = 0
    val_totalCorrect = 0
    ...
    print("Training accuracy: " +
          str(round(totalCorrect.cpu().numpy() / len(dataset) * 10000) / 100))

    # Print validation accuracy each epoch end
    print("Validation accuracy: " +
          str(round(val_totalCorrect.cpu().numpy() / len(val_dataset) * 10000) / 100))
